...
def imagenet_dataloaders(
    ...
    """
    ImageNet dataloader with proper transforms that return tensors
    """
    ...
    print("Dataset information: ImageNet")
    print("Number of train samples: ~1.28M")
    print("Number of val samples: 50K")
    print("Data augmentation: RandomResizedCrop(224) + RandomHorizontalFlip")
    ...
    # Convert targets to numpy array for processing
    train_set.targets = np.array([label for _, label in train_set.samples])
    val_set.targets = np.array([label for _, label in val_set.samples])

    # Create validation split from training set if needed
    ...
    return train_loader, val_loader, val_loader  # Return val_loader as test_loader
