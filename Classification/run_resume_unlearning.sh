...
echo "🔄 RESUME UNLEARNING EXPERIMENTS (Skip Completed)"
...
for method in "${!methods[@]}"; do
    ...
    for percent in "${percentages[@]}"; do
        ...
        if [ ! -f "$MASK_PATH" ]; then
            echo "⏭️  Skipping $method ${percent}% - mask not found"
            continue
        fi
        
        # Skip if already completed (has final model)
        if [ -f "$SAVE_DIR/model_best.pth.tar" ]; then
            echo "✅ Already completed: $method ${percent}%"
            continue
        fi
...
echo ""
echo "🎉 UNLEARNING EXPERIMENTS COMPLETED/RESUMED!"
echo "📊 Summary:"
find "$BASE_DIR/unlearning" -name "model_best.pth.tar" | wc -l | xargs echo "Total completed experiments:"
