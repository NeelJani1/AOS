...
from torchvision.datasets import ImageFolder  # <-- This import is already here from our last fix
...
def get_transform(interpolation=InterpolationMode.BICUBIC, size=512):
    transform = torch_transforms.Compose(
        [
            ...
            torch_transforms.Normalize([0.5], [0.5]), # Normalizes to [-1, 1]
        ]
    )
    return transform
...
def setup_fid_data(class_to_forget, path, image_size, interpolation="bicubic"):
    ...
    real_images_dir = os.path.join(path, f"imagenette_without_label_{class_to_forget}")

    # --- START OF MODIFICATION ---
    # Return the dataset objects directly instead of loading all images into RAM
    real_set_dataset = ImageFolder(
        real_images_dir, transform=transform
    )
    
    fake_set_dataset = Fake_Imagenette(path, class_to_forget, transform=transform)
    
    # Return the dataset objects
    return real_set_dataset, fake_set_dataset
    # --- END OF MODIFICATION ---