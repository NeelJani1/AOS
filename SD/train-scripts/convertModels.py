...
def savemodelDiffusers(name, compvis_config_file, diffusers_config_file, num, device="cpu"): # <-- ADD 'num' ARGUMENT
    # Construct the correct path using the epoch number
    if num is not None:
        checkpoint_path = f"models/{name}/{name}-epoch_{num}.pt" # <-- CORRECT PATH
    else:
         checkpoint_path = f"models/{name}/{name}.pt" # Fallback if no epoch num provided
    print(f"Loading CompVis checkpoint from: {checkpoint_path}") # Add print for verification
