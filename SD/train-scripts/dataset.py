...
def setup_model(config, ckpt, device):
    ...
    print("Loading state dict from checkpoint (on CPU)...")
    sd = torch.load(ckpt, map_location="cpu", weights_only=False)["state_dict"]

    print("Instantiating model structure (on CPU)...")
    model = instantiate_from_config(config.model)

    print("Loading weights into model...")
    m, u = model.load_state_dict(sd, strict=False)

    print("Clearing state dict from system RAM...")
    del sd

    print("Moving model to GPU...")  # CHANGED: Remove "in half-precision (fp16)"
    model = model.to(device)
    
    model.eval()
    print("Model setup complete.")
    print(f"Model dtype: {next(model.parameters()).dtype}")  # ADD: Show actual dtype
    return model
