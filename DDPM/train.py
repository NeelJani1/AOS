...
def parse_args_and_config():
    ...
    parser.add_argument(
        "--grad_accumulation", type=int, default=1, help="Gradient accumulation steps"
    )
    parser.add_argument(
        "--batch_size_override", type=int, default=None, help="Override batch size for faster training"
    )
    ...
    if args.batch_size_override and hasattr(config.training, 'batch_size'):
        config.training.batch_size = args.batch_size_override
        logging.info(f"Overriding batch size to: {args.batch_size_override}")
