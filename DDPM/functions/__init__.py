...
def get_config_and_setup_dirs(filename: str):
    ...
    # === MODIFIED: Use fixed directory for resuming ===
    resume_dir = "resume_run"  # Fixed directory name
    config.exp_root_dir = os.path.join(
        "./results", config.data.dataset.lower(), resume_dir
    )
    ...
    config_path = os.path.join(config.log_dir, "config.yaml")
